...
Since $\bS \in \S_+^n$, we can work with the ``square-root'' of $\bS$ instead (e.g., its
...
\todo{From properties of the least-squares estimator, we have that
  $(\bY - \bX\bB)^T\bX = 0$. Thus, it follows that

  \begin{align*}
\| \bS - \bX\widehat{\bW}\bX^T \|_F &=\| \bY\bY^T - \bX\widehat{\bB}\widehat{\bB}^T\bX^T \|_F \\
&=\| \bY (\bY - \bX\widehat{\bB})^T \|_F\\
&=\| (\bY - \bX\widehat{\bB} + \bX\widehat{\bB}) (\bY - \bX\widehat{\bB})^T \|_F\\
&= \|\bY - \bX\widehat{\bB}\|_F^2 .
\end{align*}
}

\todo{Although $\widehat \bW$ is not exactly a solution to the optimization $\min_{\bW}
\|\bS-\bX\bW\bX^T\|_F^2$, it is easy to relate the two optimizations as follows. Let
$\widetilde \bW$ be a solution to $\min_{\bW} \|\bS-\bX\bW\bX^T\|_F^2$ and let
$\widetilde\bB\widetilde\bB^T$ be its Cholesky factorization. Assuming the normalization
$\|\widehat\bB\|_F, \|\widetilde \bB\|_F \leq 1$, we have
...
The optimization $\min_{\bB\in \R^{p\times r}} \|\bY-\bX\bB\|_F^2$ can be modified to
