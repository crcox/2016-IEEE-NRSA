...
%Each stimulus is related to a set of features to create a word by feature matrix, where
%each feature is something like ``has wings'' or ``can swim''. Each row is populated with
%the number of times each feature is listed for that stimulus. The target semantic
%similarity matrix is derived as the cosine distance among the vectors for each of the 37
%names. We get $r = 8$ in this case to attain $\|\bS - \bY\bY^T\|_F \leq 0.3$.
...
\begin{figure}[!h]
  \centering
    ...
    \caption{Network plot showing the top edges from the $\bW$ matrix for the
      ...
      subject. The thickness of the edges is proportional to the edge weights.}
    \label{fig.NW}
\end{figure}
\fi
