...
%More intuitively, if $\beta_k$ doesn't go up by $\varepsilon$ in magnitude,
%then increase its magnitude so that it does and call this $\bv'$ with $v'_j =
%\beta_j - \varepsilon$ and $v'_k = \beta_k + \varepsilon$.  Then we apply
%Lemma~2.1 in \cite{owl} to $\bv'$ and $\Omega_{\bw}(\bv) \leq
%\Omega_{\bw}(\bv')$.
...
The following theorems state that nearly identical variables lead to equal norm
...
%We study another variant of the GrOWL which leads to exact clustering of
